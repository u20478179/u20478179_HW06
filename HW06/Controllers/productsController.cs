...
namespace HW06.Controllers
{
    public class productsController : Controller
    {
        ...
        // GET: products
        public ActionResult Index(string search, string type, string product, int? i)
        {
            return View(db.products.Where(x => x.product_name.Contains(search) || search == null).ToList().ToPagedList(i ?? 1,10));
        ...
        public ActionResult DetailsR(int id)
        {
            ...
            //product.Include(p=>p.order_items);
            //
            return new JsonResult { Data = new { id = product.product_id, name = product.product_name }, ContentType = "application/json", JsonRequestBehavior = JsonRequestBehavior.AllowGet };
        ...
        // GET: products/Edit/5
        public ActionResult Edit(int? id, int? i)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            ...
            ViewBag.brand_id = new SelectList(db.brands, "brand_id", "brand_name", product.brand_id);
            ViewBag.category_id = new SelectList(db.categories, "category_id", "category_name", product.category_id);
            return View(product)/*.ToList().ToPagedList(i ?? 1, 10)*/;
        }
